...
// TODO: move those constants to x86_64::registers::msr
...
enum ApicRegister {
    /// Local APIC ID register. Read-only. See Section 10.12.5.1 for initial
    /// values.
    XAPIC_ID = 0x020,
    /// Local APIC Version register. Read-only. Same version used in xAPIC mode
    /// and x2APIC mode.
    XAPIC_VERSION = 0x030,
    ...
    XAPIC_LDR = 0x0D0,
    /// Spurious Interrupt Vector Register (SVR). Read/write. See Section 10.9
    /// for reserved bits.
    XAPIC_SVR = 0x0F0,
    ...
    XAPIC_LVT_CMCI = 0x2F0,
    /// Interrupt Command Register (ICR). Read/write. See Figure 10-28 for
    /// reserved bits
    XAPIC_ICR0 = 0x300,
    /// Interrupt Command Register (ICR). Read/write. See Figure 10-28 for
    /// reserved bits
    ...
    XAPIC_LVT_TIMER = 0x320,
    /// LVT Thermal Sensor register. Read/write. See Figure 10-8 for reserved
    /// bits.
    XAPIC_LVT_THERMAL = 0x330,
    /// LVT Performance Monitoring register. Read/write. See Figure 10-8 for
    /// reserved bits.
    XAPIC_LVT_PMI = 0x340,
    /// LVT LINT0 register. Read/write. See Figure 10-8 for reserved bits.
    XAPIC_LVT_LINT0 = 0x350,
    /// LVT LINT1 register. Read/write. See Figure 10-8 for reserved bits.
    XAPIC_LVT_LINT1 = 0x360,
    /// LVT Error register. Read/write. See Figure 10-8 for reserved bits.
    XAPIC_LVT_ERROR = 0x370,
    /// Initial Count register (for Timer). Read/write.
    XAPIC_TIMER_INIT_COUNT = 0x380,
    /// Current Count register (for Timer). Read-only.
    XAPIC_TIMER_CURRENT_COUNT = 0x390,
    /// Divide Configuration Register (DCR; for Timer). Read/write. See Figure
    /// 10-10 for reserved bits.
    XAPIC_TIMER_DIV_CONF = 0x3E0,
}
...
impl XApic {
    ...
    pub fn new(apic_region: &'static mut [u32]) -> XApic {
        unsafe {
            XApic {
                ...
                base: Msr::new(IA32_APIC_BASE).read(),
            }
        }
    }
    ...
    pub fn attach(&mut self) {
        ...
        unsafe {
            ...
            self.base = Msr::new(IA32_APIC_BASE).read();
            self.base.set_bit(11, true);
            Msr::new(IA32_APIC_BASE).write(self.base);

            // Enable this XAPIC (set bit 8, spurious IRQ vector 15)
            let svr: u32 = 1 << 8 | 0b1111;
...
impl ApicControl for XApic {
    ...
    fn tsc_set(&self, value: u64) {
        unsafe {
            Msr::new(IA32_TSC_DEADLINE).write(value);
        }
    }
    ...
    unsafe fn send_ipi(&mut self, icr: Icr) {
        ...
        loop {
            ...
            if (icr >> 12 & 0x1) == 0 || self.read(ApicRegister::XAPIC_ESR) > 0 {
                break;
            }
        }
    }
